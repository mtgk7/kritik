...
const PREMIUM_FEATURES = [
  { icon: '🎯', title: 'AI Tahmin & Güven Skoru', desc: 'Claude AI destekli algoritma her maç için tahmin ve %0–100 güven skoru üretir.' },
  ...
  { icon: '🔬', title: 'Detaylı Maç Analizi', desc: 'Tahmin dağılımı, takım karşılaştırması ve AI algoritma yorumu.' },
  { icon: '⭐', title: 'Premium Kuponlar', desc: 'Güven skoru %70+ yüksek kombinasyon kuponları yalnızca premium üyelere açık.' },
  { icon: '✍️', title: 'Editör Hazır Kuponlar', desc: 'Uzman editörlerimizin özenle seçtiği özel kupon önerileri ve yorumları.' },
...
const PLANS = [
  { id: 'monthly',   label: 'Aylık',    price: '₺149',  period: '/ay',    note: '' },
  { id: 'quarterly', label: '3 Aylık',  price: '₺379',  period: '/3 ay',  note: '%15 indirim' },
  { id: 'annual',    label: 'Yıllık',   price: '₺1.199', period: '/yıl', note: '%33 indirim' },
]
    ...
    <div style={{ background: 'var(--color-base)' }}>
      ...
      <section style={{
        ...
        <p style={{
          ...
          xG analizleri, form skorları ve sakatlık etkileriyle güçlendirilmiş
          algoritma tahminleri. Karar vermeden önce Kritik'e bak.
        </p>
        ...
        <div style={{
          ...
          {STATS.map((s, i) => (
            <div key={i} style={{
              ...
              <div style={{ fontSize: '0.72rem', color: 'var(--color-text-on-dark-2)', marginTop: '0.3rem', letterSpacing: '0.04em' }}>
                {s.label}
              </div>
            </div>
          ))}
      ...
      <section style={{ padding: 'clamp(3rem, 6vw, 5rem) var(--page-pad)' }}>
        ...
        <div style={{
            ...
            <div style={{ padding: '1.5rem 1.75rem', display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
              {FREE_FEATURES.map((f, i) => (
                <div key={i} style={{ display: 'flex', gap: '0.85rem', alignItems: 'flex-start' }}>
                  <span style={{ fontSize: '1.1rem', flexShrink: 0, lineHeight: 1.3 }}>{f.icon}</span>
                  <div>
                    <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--color-text-primary)', marginBottom: '0.15rem' }}>{f.title}</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--color-text-tertiary)', lineHeight: 1.45 }}>{f.desc}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{
            ...
            <div style={{
              padding: '1.5rem 1.75rem', borderBottom: '1px solid oklch(85% 0.06 68 / 0.3)',
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.4rem' }}>
                ...
                <span style={{ fontSize: '0.82rem', color: 'var(--color-text-tertiary)' }}>/ay'dan başlayan</span>
              </div>
              <a href="/kayit" style={{
                display: 'block', marginTop: '1.25rem', textAlign: 'center',
      ...
      <section style={{
        ...
        <div style={{
          ...
          gap: '1rem', maxWidth: '800px', margin: '0 auto 2.5rem',
        }}>
          {PLANS.map((p, i) => (
            <a key={p.id} href="/kayit" style={{
              display: 'block', textDecoration: 'none',
              border: i === 1 ? '2px solid var(--color-premium)' : '1px solid var(--color-border)',
              borderRadius: '12px', padding: '1.5rem',
              background: i === 1 ? 'var(--color-premium-bg)' : 'var(--color-base)',
              ...
              <div style={{ fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: i === 1 ? 'var(--color-premium)' : 'var(--color-text-tertiary)', marginBottom: '0.75rem' }}>
                {p.label}
              </div>
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '2rem', lineHeight: 1, color: i === 1 ? 'var(--color-premium)' : 'var(--color-text-primary)', letterSpacing: '-0.01em' }}>
                {p.price}
              </div>
              <div style={{ fontSize: '0.72rem', color: 'var(--color-text-tertiary)', marginTop: '0.25rem' }}>
                {p.period}
              </div>
            </a>
          ))}
